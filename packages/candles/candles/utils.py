...
def morph_prev_base_timeseries_obj(new_obj: TimeseriesObject, prev_obj: TimeseriesObject):
    """
    Morphs a previous TimeseriesObject to match the base timeframe of a new TimeseriesObject, if the base timeframes differ.
    ...
    """
    if prev_obj.base_timeframe != new_obj.base_timeframe:
        if prev_obj.base_timeframe.ms % new_obj.base_timeframe.ms != 0:
            raise ValueError(
                f"Cannot morph previous base timeframe {prev_obj.base_timeframe} into new base timeframe {new_obj.base_timeframe}. "
                f"Base timeframes must be divisible but got {prev_obj.base_timeframe.ms} and {new_obj.base_timeframe.ms}."
            )
        ...
        if prev_obj.timestamp + prev_obj.base_timeframe.ms != new_obj.timestamp:
            ...
            raise ValueError(
                ...
                f"I.e. previous object's timestamp + base timeframe interval must equal new object's timestamp. "
